...
function createExtendedState(spec) {
    function setupParallelMachines() {
        ...
        tasks.forEach(task => {
            ...
            // let progressStartingValue = [task.initial];
            // if (task.id === superSpec.routes.initial) {
            //     // only the first task's initial value should be appended to the array.
            //     progressStartingValue = [task.initial];
            // }
            // taskSpec.progress = task.progress ? task.progress : progressStartingValue;
