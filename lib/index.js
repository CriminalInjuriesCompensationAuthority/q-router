    ...
    function findCascadeIndex(sectionId) {
        ...
        for (let i = startIndex, sectionRoutes; i < q.progress.length; i += 1) {
            ...
            if (sectionRoutes && 'on' in sectionRoutes) {
                const onProps = Object.keys(sectionRoutes.on);
                const targets = sectionRoutes.on[onProps.find(prop => prop.startsWith('ANSWER'))];
                ...
                if (!sameAnswer) {
                    ...
                    if (cascadeIndex > -1) {
                        ...
                        if (fireEventAtAllMachines) {
                            const newSpec = fireEventAtAllMachines(
                                `CASCADE__${q.id.toUpperCase()}`
                            );
                            q.currentSectionId = newSpec.routes.states[q.id].currentSectionId;
                            q.progress = newSpec.routes.states[q.id].progress;
                        }
                    }
                }
            }
