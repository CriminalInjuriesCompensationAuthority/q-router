...
*/
    ...
    function next(answers, currentSectionId = q.currentSectionId /* , event = 'ANSWER' */) {
        ...
        if (answers !== undefined) {
            const {context} = xStateMachine.getContext();
            let hasExistingAnswer = false;
            let existingAnswers = {};
            context.tasks.forEach(task => {
                if (task.answers[currentSectionId] !== undefined) {
                    hasExistingAnswer = true;
                    existingAnswers = task.answers[currentSectionId];
                }
            });
            // If there is no current answer save the answer
            if (hasExistingAnswer) {
                const sameAnswer = deepEqual(answers, existingAnswers);
        ...
        // Add new answer(s)
        return xStateMachine.transition('ANSWER', answers).value;
    }
