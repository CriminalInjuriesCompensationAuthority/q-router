...
function qRouter(spec) {
    ...
    function current(currentSectionId) {
        if (currentSectionId && currentSectionId !== q.current.id) {
            if (!isSectionAvailable(currentSectionId)) {
                return undefined;
            }

            q.current.id = currentSectionId;
        }

        return q.current;
    }

    function next(answers, currentSectionId = q.current.id, event = 'ANSWER') {
        if (!isSectionAvailable(currentSectionId)) {
            return undefined;
        }
    ...
    function previous(currentSectionId = q.current.id) {
        if (!isSectionAvailable(currentSectionId)) {
            return undefined;
        }
        ...
        q.current.id = q.progress[currentIndex - 1];
