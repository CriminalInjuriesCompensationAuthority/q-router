...
function qRouter(spec) {
    ...
    function getAllRoleRoutingMatches(target, sectionId) {
        const userRoles = target;
        let matches = [];
        for (let userRole = 1; userRole < userRoles.length; userRole += 1) {
            if (Array.isArray(userRoles[userRole])) {
                matches = [...matches, ...getAllRoleRoutingMatches(userRoles[userRole], sectionId)];
            } else {
                const roleKey = userRoles[userRole];
                if (isPageAndQuestion(roleKey)) {
                    if (roleKey.includes(sectionId)) {
                        matches.push(roleKey);
                    }
                    userRole = userRoles.length;
                } else if (
                    roles[roleKey].schema.const.some(element =>
                        String(element).includes(`.${sectionId}.`)
                    )
                ) {
                    matches.push(roleKey);
    ...
    function findCascadeIndex(sectionId) {
        const startIndex = 0; // q.progress.indexOf(sectionId);
            ...
            if (sectionRoutes && 'on' in sectionRoutes) {
                ...
                if (Array.isArray(targets)) {
                    const cascade = targets.some(target => {
                        if ('cond' in target) {
                            // const matchings = getAllRoleRoutingMatches(target.cond, sectionId);
                            // if (matchings.length) {
                            //     // console.log({
                            //     //     sectionId,
                            //     //     matches: matchings,
                            //     //     isUsingRoleRouting: isUsingRoleRouting(target),
                            //     //     hasRoleRoutingMatch: hasRoleRoutingMatch(
                            //     //         target.cond,
                            //     //         sectionId
                            //     //     ),
                            //     //     targetIncludes: target.cond.some(element =>
                            //     //         String(element).includes(`.${sectionId}.`)
                            //     //     )
                            //     // });
                            // }
