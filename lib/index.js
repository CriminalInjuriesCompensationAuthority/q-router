                ...
                if (Array.isArray(targets)) {
                    const cascade = targets.some(target => {
                        if ('cond' in target) {
                            ...
                            if (target.cond.some(element => String(element).includes('|role'))) {
                                const roleConditions = target.cond;
                                for (
                                    let conditions = 1, len = roleConditions.length;
                                    conditions < len;
                                    conditions += 1
                                ) {
                                    const roleKey = roleConditions[conditions];
                                    if (
