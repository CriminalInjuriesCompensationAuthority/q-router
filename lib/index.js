...
function qRouter(spec) {
    ...
    q.answers = q.answers || {};
    q.usedSchemas = q.usedSchemas || {};
    q.retractedAnswers = q.retractedAnswers || {};
    q.retractedSchemas = q.retractedSchemas || {};
    ...
    function restoreSchema(sectionId) {
        const schemaToRestore = q.retractedSchemas[sectionId];

        if (schemaToRestore !== undefined) {
            q.usedSchemas[sectionId] = schemaToRestore;
            delete q.retractedSchemas[sectionId];
        }

        return schemaToRestore;
    }
    ...
    function retractSchemas(sectionIds) {
        const retractedSchemas = [];

        sectionIds.forEach(sectionId => {
            const currentSchema = q.usedSchemas[sectionId];

            if (currentSchema !== undefined) {
                q.retractedSchemas[sectionId] = currentSchema;
                delete q.usedSchemas[sectionId];

                retractedSchemas.push(currentSchema);
            }
        });

        return retractedSchemas;
    }
    ...
    function removeProgress(startIndex) {
        ...
        retractSchemas(removedSectionIds);
    ...
    function next(answeredSchema, currentSectionId = q.currentSectionId, event = 'ANSWER') {
        const {answers} = answeredSchema;
        const {schema} = answeredSchema;
        if (!isSectionAvailable(currentSectionId)) {
            return undefined;
        }
        ...
        if (answers !== undefined) {
            // If there is no current answer save the answer and answered schema
            if (q.answers[currentSectionId] === undefined) {
                q.answers[currentSectionId] = answers;
                q.usedSchemas[currentSectionId] = schema;
            } else {
