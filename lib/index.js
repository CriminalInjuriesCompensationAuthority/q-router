...
function qRouter(spec, qExpressions = defaults.qExpressions) {
    ...
    function checkTaskCascade(sectionId) {
        // Check if the applicability of a task depends on a specific question
        const applicabilityRoutes = q.routes.states;
        if (!applicabilityRoutes) {
            return false;
        }
        return Object.keys(applicabilityRoutes).some(route => {
            const targets = applicabilityRoutes[route].on?.UPDATE__STATUS;
            if (!targets) {
                return false;
            }
            return targets.some(target => {
                ...
                if (isUsingRoleRouting(target)) {
                    return hasRoleRoutingMatch(cond, sectionId);
                }
                return cond.some(element => String(element).includes(`.${sectionId}.`));
            });
        });
    }
