...
function qRouter(spec) {
    ...
    // if no history exists create an array to store them against
    extendedState.history = extendedState.history || [];

    // If available, use the current progress (history) as the initialState
    const historyLength = extendedState.history.length;
    let currentState =
        historyLength === 0
            ? machine.initialState
            : {value: extendedState.history[historyLength - 1]};
    ...
    function next(event = '', values = {}) {
        ...
        extendedState.history.push(stateId);
        currentState = machine.transition(currentState, event, extendedState);
    }

    function previous() {
        currentState = {value: extendedState.history.pop()};
    }
