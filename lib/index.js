...
function qRouter(spec) {
    ...
    function can(sectionId) {
        const possibleRoutes = spec.routes[sectionId].on.ANSWER;
        const validRoutes = [];
        possibleRoutes.forEach(route => {
            if (!route.target) {
                throw new Error(`All routes for "${sectionId}" must include a target`);
            }
            if (route.cond) {
                if (qExpression.evaluate(route.cond, spec)) {
                    validRoutes.push(route.target);
                }
            } else {
                validRoutes.push(route.target);
            }
