...
function qRouter(spec) {
    ...
    function retractAnswers(sectionIds) {
        ...
        sectionIds.forEach(sectionId => {
            // undo other things
            const route = q.routes.states[sectionId];

            if (route.entry) {
                q.answers[`${machineId}-status`] = 'incomplete';
            }
    ...
    function next(answers, currentSectionId = q.currentSectionId, event = 'ANSWER') {
        ...
        // Process actions
        if (state.actions) {
            state.actions.forEach(actions => {
                if (actions.type === 'complete') {
                    q.answers[`${machineId}-status`] = 'complete';
                }
