...
function qRouter(spec, questionnaireRoles) {
    const q = {...spec};
    const fsm = createMachine(q.routes);
    const roles = questionnaireRoles || q.attributes?.q__roles;
    q.answers = q.answers || {};
        ...
        for (let i = startIndex, sectionRoutes; i < q.progress.length; i += 1) {
            ...
            if ('on' in sectionRoutes) {
                ...
                if (Array.isArray(targets)) {
                    ...
                    if (cascade) {
                        console.log({a: q.status, b: q.id});
                        q.status[q.id] = 'incomplete';
                        // If startIndex and i are equal, this is a section that relies on its own answer(s)
                        // for routing. Don't remove it from the progress, but everything after it (i + 1).
            ...
            state.actions.forEach(action => {
                ...
                if (action.type === '|updateStatus') {
                    ...
                    if (newDestinationValue !== undefined) {
                        q.status[actionDestination] = newDestinationValue;
                    }
                }
                ...
                // if (actions.type === 'raise:d2') {
                //     events.push({type: 'd2', source: machineId});
                // }

                // if (actions.type === 'raise:g') {
                //     events.push({type: 'g', source: machineId});
                // }
            });
        }
