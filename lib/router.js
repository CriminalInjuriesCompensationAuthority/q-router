...
function qRouter(spec) {
    ...
    function next(answers, currentSectionId = q.currentSectionId, event = 'ANSWER', superContext) {
        if (!isSectionAvailable(currentSectionId)) {
            return undefined;
        }
        ...
        const transitionContext = superContext || q;

        const state = fsm.transition({value: currentSectionId}, event, transitionContext);
