...
function qRouter(spec) {
    ...
    function findCascadeIndex2(sectionId) {
        ...
        for (let i = startIndex, sectionRoutes; i < q.progress.length; i += 1) {
            ...
            // If there is an "on" attribute there may be conditions -- The task list's routes are unknown to it's child machine's, so ignore undefined routes
    ...
    function isQuestionFinalInTask(sectionId) {
        return (
            q.routes.states[sectionId].on.ANSWER &&
            q.routes.states[sectionId].on.ANSWER.some(
                target => target.type && target.type === 'final'
            )
        );
    }
    ...
    function next2(answers, currentSectionId = q.currentSectionId, event = 'ANSWER', superContext) {
        ...
        if (isQuestionFinalInTask(currentSectionId)) {
            q.status = 'complete';
        }
    ...
    function externalCascade(question) {
        const startIndex = q.progress.indexOf(question) + 1;
        removeProgress(startIndex);
        q.status = 'incomplete';
        q.events.push({type: 'cascade', value: question, origin: q.id});
        return q;
    }
