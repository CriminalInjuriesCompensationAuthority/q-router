...
function createParallelRouter(spec) {
    ...
    function processImplicitEvents(states) {
        const newEvents = [];
        if ('events' in states && states.events.length > 0) {
            states.events.forEach(event => {
    ...
    function fireEventAtAllMachines(event) {
        ...
        for (let i = 0; i < machines.length; i += 1) {
            ...
            try {
                ...
                if (state !== undefined) {
                    pq.currentSectionId = state.context.currentSectionId;
    ...
    function next(answers, sectionId, event = `ANSWER__${sectionId.toUpperCase()}`) {
        ...
        if (isMachineId(sectionId)) {
            ...
            if (isMachineId(nextResponse.id)) {
                ...
                pq = fireEventAtAllMachines(`COMPLETE__${originMachineId.toUpperCase()}`);
                nextResponse = machine.machine.current();
            }
            pq = fireEventAtAllMachines(event);
            pq = fireEventAtAllMachines(`ANSWER__${machine.id.toUpperCase()}`);
