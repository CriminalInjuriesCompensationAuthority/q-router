    ...
    function processImplicitEvents(objectWithEvents) {
        ...
        const processEvent = event => {
            ...
            if (event.type === 'cascade') {
                ...
                // Any cascade should cause the CYA task to be reset, as that can only be reached once all other applicable tasks are complete
                const cyaTask = 't-check-your-answers';
                const cyaPage = pq.routes.states[cyaTask].initial;
                pq.routes.states[cyaTask].progress = [cyaPage];
                pq.routes.states[cyaTask].currentSectionId = cyaPage;
                pq.attributes.q__statuses[cyaTask] = cyaPage;
