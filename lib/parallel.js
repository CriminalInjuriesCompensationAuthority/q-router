...
function createParallelRouter(spec) {
    ...
    function fireEventAtAllMachines(event) {
        ...
        for (let i = 0; i < machines.length; i += 1) {
            ...
            try {
                const state = machines[i].machine.next({}, machineCurrentSectionId, event);
    ...
    function processImplicitEvents(objectWithEvents) {
        ...
        if ('events' in objectWithEvents && objectWithEvents.events.length > 0) {
            objectWithEvents.events.forEach(event => {
                if (event.eventName !== 'UPDATE__STATUS') {
                ...
                if ('type' in event) {
                    if (event.type === 'cascade') {
                        ...
                        //     }) + 1
                        // );
                        // console.log({machineslength: machines.length});
                        machines.forEach(machine => {
                            ...
                            if (cascadeIndex > -1) {
                                ...
                                //     type: 'fire-event',
                                //     eventName: `CASCADE__${machine.id.toUpperCase()}`,
                                //     sourceId: machine.id
                                // });

                                const implicitEvents = removedIds.flatMap(removedId => {
                                    console.log({
                                        removedId,
                                        machine: getMachineWithSpecificPageId(removedId)
                                    });
                                    return [
                                        {
                                            type: 'cascade',
                                        ...
                                        {
                                            type: 'fire-event',
                                            eventName: `CASCADE__${getMachineWithSpecificPageId(
                                                removedId
                                            ).id.toUpperCase()}`,
                                            sourceId: machine.id
                                        }
    ...
    function next(answers, sectionId, event = 'ANSWER') {
        ...
        pq.routes.states[machine.id].progress = nextResponse.context.progress.filter(
            progressItem => !isMachineId(progressItem)
        );
