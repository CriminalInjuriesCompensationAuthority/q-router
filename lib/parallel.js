...
function createParallelRouter(spec) {
    ...
    function getStatuses() {
        const statuses = {};
        Object.keys(spec.routes.states).forEach(task => {
            statuses[task] = spec.routes.states[task].currentSectionId;
        });
        return statuses;
    }
            ...
            objectWithEvents.events.forEach(event => {
                if ('type' in event) {
                    ...
                    if (event.type === 'fire-event') {
                        pq = fireEventAtAllMachines(event.eventName);
                    }
                }
            });
        }
