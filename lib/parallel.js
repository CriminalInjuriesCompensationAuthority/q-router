...
const defaults = {};
defaults.qExpressions = require('q-expressions-parallel');

function createParallelRouter(spec, qExpressions = defaults.qExpressions) {
    function removeInapplicableAnswers(questionnaire) {
        const q = {...questionnaire};
        const {states} = q.routes;
        Object.keys(states).forEach(task => {
            const applicabilityStatus = `${task}__applicability-status`;
            const completionStatus = `${task}__completion-status`;
            if (states[applicabilityStatus]?.currentSectionId === 'notApplicable') {
                // Retract the inapplicable answers
                states[task].progress.forEach((sectionId, index) => {
                    if (index === 0) {
                        return;
                    }
                    q.retractedAnswers[sectionId] = q.answers[sectionId];
                    delete q.answers[sectionId];
                });
                // Reset the progress
                states[task].progress = [states[task].initial];
                states[task].currentSectionId = states[task].initial;
                // Reset the completion status as well
                if (states[completionStatus]?.currentSectionId === 'completed') {
                    states[completionStatus].currentSectionId = states[completionStatus].initial;
                    states[completionStatus].progress = [states[completionStatus].progress[0]];
                    q.attributes.q__statuses[completionStatus] =
                        states[completionStatus].currentSectionId;
                }
            }
        });
        return q;
    ...
    const pq = removeInapplicableAnswers({...spec});
    const machines = [];
    ...
    function getValue(verbose = false) {
        ...
        for (let i = 0; i < machines.length; i += 1) {
            ...
            try {
                ...
                if (state !== undefined) {
                    ...
                    pq.attributes.q__statuses[machines[i].id] = state.context.currentSectionId;
                }
            } catch (err) {
                if (
    ...
    function processRemovedIds(removedIds, machineId) {
        ...
        const specificPageId = getMachineWithSpecificPageId(removedIds[0]).id.toUpperCase();
        implicitEvents.push({
            type: 'fire-event',
        ...
        const processEvent = event => {
            ...
            if (event.type === 'cascade') {
                machines.forEach(machine => {
                    if (machine.id.includes('__applicability-status')) {
                        // Check if a cascade impacts the applicability of a task
                        if (machine.machine.checkTaskCascade(event.questionId)) {
                            const machineIdWithQuestions = machine.id.replace(
                                '__applicability-status',
                                ''
                            );
                            const taskMachine = machines.find(m => m.id === machineIdWithQuestions);
                            // All questions must be removed from the progress of a task if it changes applicability status
                            const removedIds = taskMachine.machine
                                .removeProgress(0)
                                .filter(progressItem => !isMachineId(progressItem));

                            // If questions were removed, they must also be checked for cascades
                            const implicitEvents = processRemovedIds(removedIds, machine.id);
                            eventsQueue.push(...implicitEvents);
                        }
                        return;
                    }
                    // Check for cascades caused by direct references to questions
                    const cascadeIndex = machine.machine.findCascadeIndex(
                        event.questionId,
                        machine.id
                    );
                    if (cascadeIndex === -1) {
    ...
    if (JSON.stringify(pq.attributes.q__statuses) === '{}') {
        pq.attributes.q__statuses = getValue(true);
    }
